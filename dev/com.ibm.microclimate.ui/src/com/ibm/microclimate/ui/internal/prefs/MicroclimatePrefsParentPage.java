	...
	public boolean performOk() {
		...
		if (this.webBrowserCombo != null) {
			...
			if (webBrowserCombo.getSelectionIndex() > 0) {
				...
				if (browserName != null) {
					prefs.setValue(MicroclimateCorePlugin.NODEJS_DEBUG_BROWSER_PREFSKEY, browserName);
				}
			} else {
				prefs.setToDefault(MicroclimateCorePlugin.NODEJS_DEBUG_BROWSER_PREFSKEY);
			}
		}
	...
	protected void refreshPreferencesPage() {
        ...
        if (bm != null) {
	        ...
	        if (browserList != null) {
				...
				MCLogger.log("Refresh preference page valid browser length: " + len);  //$NON-NLS-1$
		        		        		        
				// When it loads, check if preference exists
				String foundDefaultBrowser = prefs.getString(MicroclimateCorePlugin.NODEJS_DEBUG_BROWSER_PREFSKEY);
				...
				MCLogger.log("Refresh preference page found default browser: " + foundDefaultBrowser);  //$NON-NLS-1$
		        
				if (foundDefaultBrowser == null || !foundDefaultBrowserName){
					// Could not find browser from preference or browser got removed
					MCLogger.log("Refresh preference page: could not find preferences or browser was removed");  //$NON-NLS-1$
					prefs.setToDefault(MicroclimateCorePlugin.NODEJS_DEBUG_BROWSER_PREFSKEY);
				}
